...
// Yarn v4 requires Node 18+
const nodeVersion = semver.parse(process.version)?.major;
if (nodeVersion && nodeVersion >= 18) {
	describe("End to end tests - yarn v4", () => {
		...
		it("overriding dependencies works", async () => {
			...
			let version = await spawn("node", ["-p", 'require("is-odd/package.json").version'], {
				cwd: testDir,
			});
		...
		it.skip("does not install devDependencies, unless the environment is set to development", async () => {
			...
			await yarn.install([]);
			const version = await spawn("node", ["-p", 'require("is-odd/package.json").version'], {
				cwd: testDir,
			});
			// now it is
